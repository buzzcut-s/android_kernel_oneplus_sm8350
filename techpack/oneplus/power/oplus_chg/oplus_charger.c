...
static void oplus_mmi_fastchg_in(struct oplus_chg_chip *chip)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chip->mmi_adapter_in_work, round_jiffies_relative(msecs_to_jiffies(2000)));
}
...
static void oplus_chg_update_work(struct work_struct *work)
{
	...
	if (!timer_pending(&chip->update_work.timer)) {
		...
		queue_delayed_work(system_power_efficient_wq, &chip->update_work, OPLUS_CHG_UPDATE_INTERVAL(oplus_chg_update_slow(chip)));
	}
#else
